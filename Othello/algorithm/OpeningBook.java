...
public class OpeningBook {
    ...
    String[] openings = {
        ...
        public ArrayList<ArrayList<Point>> list;

        public void initOpening(){
            list = new ArrayList<>();
            for(String sequence : openings){
                String editSeq = sequence.toLowerCase();
                ArrayList<Point> tracksequence = new ArrayList<>();
                for (int i = 0; i < editSeq.length(); i+=2) {
                    char c1 = editSeq.charAt(i);
                    char c2 = editSeq.charAt(i+1);
                    int col = c1 - 'a';
                    int row = c2 - '1';
                    tracksequence.add(new Point(row,col));
                }
                list.add(tracksequence);
            }
        }

        public Point move(ArrayList<Point> history){
            ArrayList<ArrayList<Point>> endTrack = new ArrayList<>();
            ArrayList<Point> availableTransitions = new ArrayList<>();
            for(ArrayList<Point> sequence : list){
                if(sequence.size() <= history.size()){
                    //sequence is no more used and considered expired
                    endTrack.add(sequence);
                    continue;
                }
                int i;
                ...
                if(isMatch){
                    availableTransitions.add(sequence.get(i));
                }
            }
            //dispose expired sequences
            list.removeAll(endTrack);

            if(availableTransitions.size()>0) {
                //choose randomly between available moves
                return availableTransitions.get(random.nextInt(availableTransitions.size()));
            }else return null;
        }
